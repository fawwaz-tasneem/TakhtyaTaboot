...
| Coronation darbar HELD IN THE HALL (summons → keep menu → lords materialised in the lord's hall swear in dialogue; courier fallback after 14 days; vassal swears in person) | `CoronationBehavior` (ruler snapshot; `LocationCharacter` stand-ins via the native keep-notable recipe, mission-scoped state never serialized; AI silent) | `CoronationMath` (attendance, late oath, deadline, oath register) | `hind_coron_*` (incl. `_pend_*`, `_oath_*`) | `coronation_test` |
