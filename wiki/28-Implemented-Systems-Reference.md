...
| Coronation darbar HELD IN THE HALL (summons → keep menu → lords materialised in the hall; the PROCESSION walks each lord to the sovereign to swear in his culture's voice, 7 variations × 8 cultures; courier fallback after 14 days; vassal swears in person) | `CoronationBehavior` (ruler snapshot; `LocationCharacter` stand-ins, mission-scoped state never serialized; AI silent) + `Util/CoronationProcessionLogic` (MissionBehavior, added only during a live ceremony) | `CoronationMath` (attendance, late oath, deadline, register), `CoronationOaths` (the oath book) | `hind_coron_*` (incl. `_pend_*`, `_oath_*`) | `coronation_test` |
...
| ENGLISH OVERRIDE FIX (the engine never loads language files for English — a Harmony prefix on `MBTextManager.GetLocalizedText` serves the mod's ~700 inner-key overrides at render time; heals existing saves' names/quest titles) | `Patches/EnglishTextOverridePatch` | — | — | — |
| Scripted history (real events fired once by AD year: Deccan war, Banda's rising 1709, Deccan treaty 1714, Hyder's coup 1724, Bajirao's Delhi dash 1737, Nadir's sack 1739, Durrani rise 1747; Mysore house restructure; Tipu accession → the yellow/black LION STANDARD) | `ScriptedHistoryBehavior` | `ScriptedHistory` (pure timeline, 4 tests), `HistoricalCalendar` (+Hijri) | `hind_hist_*` | `history_status`, `history_fire`, `mysore_banner` |
| Court honours (khil'at robes/char-aina in dialogue; granted titles Bahadur/Jang/ud-Daula/ul-Mulk appended via NameWithHonorific; monthly jharokha darshan menu) | `CourtHonoursBehavior` | — | `hind_honour_*` | — |
| Road-news anecdotes ("What word do the roads carry?" — 14 historical tales, weekly rotation per teller) | `HindostanDialogsBehavior` flow 8 | `HistoricalAnecdotes` (4 tests) | — | — |
...
| Darbar petition court AS DIALOGUE (plaintiff → defendant → advisor conversations, spoken judgment → CourtRuling records) | `DarbarPetitionBehavior` (conversation chain via tick pump; sitting state transient, never saved) | `DarbarCourtMath` | `hind_darbar_*` | `darbar_petition` |
| Akhbaar encyclopedia surface (dispatch button on hero page + last-reported row) | `UI/EncyclopediaHeroAkhbaarMixin` (UIExtenderEx) + `AkhbaarScoutBehavior` register | `AkhbaarMath` | `hind_akhbaar_reg*` | — |
